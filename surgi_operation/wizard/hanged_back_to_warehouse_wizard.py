...
class wizard_back_to_warehouse(models.TransientModel):
    _name = "wizard_hanged_back_to_warehouse"
    ...
    def do_transfer_selected_lines(self):
        ...
        self.seq_no = self.env['ir.sequence'].get('hanged_back_to_warehouse_number')
    ...
    def _get_location_dest_id(self):
        ...
        location_dest_id = self.env['stock.location'].search([('location_id.name', '=', parent_location.name), ('usage', '=', 'internal')])
    ...
    stk_quant_ids = fields.One2many('stock.quant.items', 'wizard_id_hanged_back', string="Stock Quant Items", required=False,
