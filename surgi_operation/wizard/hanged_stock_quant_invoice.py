...
class hanged_stock_quant_invoice(models.TransientModel):
    _name = "hanged.stock.quant.invoice"
    ...
    def get_stock_quant_lines(self):
        ...
        for stk in stock_ids:
            for line in stk:
                res = self.env['stock.quant.items.invoice'].create({
    ...
    stk_quant_ids = fields.One2many('stock.quant.items.invoice', 'wizard_id_hanged_invoice', string="Stock Quant Items", required=False, default=get_stock_quant_lines)
...
class stock_quant_details_items(models.TransientModel):
    _name = 'stock.quant.items.invoice'
    _description = 'Stock Quant Items Invoice'

    product_id = fields.Many2one('product.product', string="Product")
    quant_id = fields.Many2one('hanged.stock.quant', string="Quant")
    quantity = fields.Float(string="Qty")
    location_id = fields.Many2one('stock.location', string="Location", readonly=True)
    wizard_id_hanged_invoice = fields.Many2one('hanged.stock.quant.invoice', string="Wizard Name")